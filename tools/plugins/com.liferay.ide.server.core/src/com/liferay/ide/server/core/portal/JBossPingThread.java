...
public class JBossPingThread extends PingThread
{
    ...
    public JBossPingThread( IServer server, String url, PortalServerBehavior behaviour )
    {
        ...
        try
        {
            ...
            if( os.indexOf( "win" ) >= 0 )
            {
                serverStateCommand = new ProcessBuilder(
                    cmdDirectory.append( "jboss-cli.bat" ).toOSString(), "-c",
            ...
            {
                serverStateCommand = new ProcessBuilder(
                    cmdDirectory.append( "jboss-cli.sh" ).toOSString(), "-c",
                    "--commands=\":read-attribute(name=server-state)\"" );
            }
        }
        catch( Exception e )
            ...
            try
            {
                ...
                if( serverStateCommand != null )
                {
                    ...
                    try(BufferedReader reader =
                        new BufferedReader( new InputStreamReader( serverStateProcess.getInputStream() ) ))
                    {
                        ...
                        if( result != null && result.length() > 0 )
                        {
                            if( !result.startsWith( "Failed to connect" ) )
                            {
                                ...
                                String state = jsonResult.get( "result" );

                                if( !stop && state != null && state.equals( "running" ) )
                                {

...
public class SelectImportsDialog extends TitleAreaDialog {
	...
	private void createImportButtons(Composite container) {
		...
		Composite bcomp = SWTFactory.createComposite(container, container.getFont(), 1, 1,
	...
	private void addPackage() {
		...
		try {
			...
			dialog = JDIDebugUIPlugin.createAllPackagesDialog(shell,
					projects.toArray(new IJavaProject[projects.size()]), false);
	...
	private void addType() {
		...
		try {
			dialog = JavaUI.createTypeDialog(shell, PlatformUI.getWorkbench().getProgressService(),
					SearchEngine.createJavaSearchScope(new IJavaElement[] { fEditor.getJavaProject() }, true),
	...
	protected Control createDialogArea(Composite parent) {
		setTitle(NLS.bind(
				SnippetMessages.getString("SelectImportsDialog.Manage_the_Java_Snippet_Editor_Imports_for___{0}__1"), //$NON-NLS-1$
				new String[] { fEditor.getEditorInput().getName() }));
		setMessage(NLS.bind(SnippetMessages.getString("SelectImportsDialog.add_remove_imports"), //$NON-NLS-1$
				new String[] { fEditor.getEditorInput().getName() }));

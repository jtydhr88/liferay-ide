        ...
        ContentAssistInvocationContext context, IProgressMonitor monitor )
    {
        ...
        try
        {
            ...
            if ( candidate != null && candidate.length() > 0 )
            {
                ...
                for( int i =0; i < CODE_ASSISTANT_RESOURCE.length-1; i++)
                {
                    final String[] propertyAssist = CODE_ASSISTANT_RESOURCE[i];
                    final String propertyKey = propertyAssist[0];
                    final String propertyComment = propertyAssist[2];
                    
                    if ( ( candidate.length() > 0 ) && ( !propertyKey.contains( candidate ) ) )
                    {
                        continue;
                    }

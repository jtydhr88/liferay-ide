...
public class GogoBundleDeployer {
	...
	private GogoBundleDeployer(String host, int port) {

		_host = host;
		_port = port;
	}

	private GogoBundleDeployer() {
	...
	private BundleDTO[] _parseBundleInfos(String content) {
		...
		for (int i = 0; i < bundles.length; i++) {
			...
			bundle.symbolicName =
				infos[3].substring(0, infos[3].indexOf("(")).trim();
			bundle.version = infos[3].substring(
				infos[3].indexOf("(") + 1, infos[3].indexOf(")")).trim();
	 ...
	 */
	private String[] _split(String string, String regex) {

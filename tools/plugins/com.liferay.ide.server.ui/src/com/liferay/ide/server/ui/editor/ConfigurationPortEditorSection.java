...
public class ConfigurationPortEditorSection extends ServerEditorSection
{
    ...
    private boolean validPort( LiferayServerPort serverPort )
    {
        try
        {
            ...
            if( port < 0 || port > 65535 )
            {
                getManagedForm().getMessageManager().addMessage(
                    String.valueOf( port ), "Port must to be a number from 1~65535.", null, IMessageProvider.ERROR );
            ...
            {
                getManagedForm().getMessageManager().addMessage(
                    String.valueOf( port ), "Port is being used.", null, IMessageProvider.ERROR );
                return false;
            }
        }
        catch( Exception e )
        {
            getManagedForm().getMessageManager().addMessage(
                String.valueOf( serverPort.getPort() ), "Port is invalid", null, IMessageProvider.ERROR );

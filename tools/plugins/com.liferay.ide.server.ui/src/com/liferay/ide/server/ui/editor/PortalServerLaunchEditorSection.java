...
public class PortalServerLaunchEditorSection extends AbstractPortalServerEditorSection {
	...
	protected void validate() {
		if (portalServer != null) {
			...
			boolean memoryArgInvalid = Stream.of(
				...
				arg -> !arg.startsWith("-Xmx")
			).findAny(
			).isPresent();

			if (memoryArgInvalid) {
				setErrorMessage(Msgs.memoryArgumentPrefixInvalid);

				return;
			}

			long memorySetCount = Stream.of(
				memoryArrays
			).filter(
				arg -> arg.startsWith("-Xmx")
			).count();
			...
			if (memorySetCount > 1) {
				setErrorMessage(Msgs.memoryArgumentCountInvalid);
	...
	private static class Msgs extends NLS {
		...
		public static String memoryArgumentCountInvalid;
		public static String memoryArgumentPrefixInvalid;

...
public class ConfigurationPortEditorSection extends ServerEditorSection {
		...
		for (int i = 0; i < items.length; i++) {
			try {
				...
				if (serverPort.getId().equals(id)) {
					final LiferayServerPort changedPort = new LiferayServerPort(id, serverPort.getName(), port,
							serverPort.getProtocol(), serverPort.getStoreLocation());
	...
	public void createSection(Composite parent) {
		...
		final Section section = toolkit.createSection(parent, ExpandableComposite.TWISTIE | ExpandableComposite.EXPANDED
		...
		setDefault.addHyperlinkListener(new HyperlinkAdapter() {
			public void linkActivated(HyperlinkEvent e) {
				List<LiferayServerPort> defaultPorts = portalBundleConfiguration
						.readDefaultPorts(portalRuntime.getPortalBundle());

				for (LiferayServerPort port : defaultPorts) {
					execute(new ModifyPortCommand(portalBundleConfiguration, portalSeverDelgate, port.getId(), port.getPort()));
				}
			}
		});
		initialize();
	...
	public void init(IEditorSite site, IEditorInput input) {
		...
		portalRuntime = (PortalRuntime) server.getRuntime().loadAdapter(PortalRuntime.class, new NullProgressMonitor());
		...
		ServerCore.addServerLifecycleListener(new ServerLifecycleAdapter() {
			public void serverRemoved(IServer server) {
				IPath defaultPortsJson = _pluginPath.append(server.getId().replace(" ", "_") + "_default_ports.json");
	...
	private boolean validPort(TableItem item, LiferayServerPort serverPort) {
		...
		if (port < 0 || port > 65535) {
			getManagedForm().getMessageManager().addMessage(item, "Port must to be a number from 1~65535.", item,

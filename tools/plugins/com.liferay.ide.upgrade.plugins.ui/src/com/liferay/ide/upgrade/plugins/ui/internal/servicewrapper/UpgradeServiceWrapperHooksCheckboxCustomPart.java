 ...
 * @author Seiphon Wang
 */
public class UpgradeServiceWrapperHooksCheckboxCustomPart extends ProjectsCheckboxCustomPart {

	@Override
	public void dispose() {
		if (_listener != null) {
			Value<Object> sdkProperty = _op().property(UpgradeServiceWrapperHooksOp.PROP_SDK_LOCATION);
	...
	protected void init() {
		_listener = new FilteredListener<PropertyContentEvent>() {
			...
			protected void handleTypedEvent(PropertyContentEvent event) {
				...
				if (eventDef.equals(UpgradeServiceWrapperHooksOp.PROP_SDK_LOCATION)) {
		...
		Value<Object> sdkLocation = _op().property(UpgradeServiceWrapperHooksOp.PROP_SDK_LOCATION);
	...
	private Object[] _getProjectRecords() {
		...
		for (Object project : _selectedProjects) {
			...
			if (!_isLegacyHookProject(projectRecord.getProjectName())) {
	...
	private boolean _isLegacyHookProject(String projectName) {
		if (projectName == null) {
			return false;
		}
		...
		for (IProject project : _getPluginSDKProjects()) {
			if (FileUtil.nameEquals(project, projectName)) {
				IFile file = project.getFile("docroot/WEB-INF/liferay-hook.xml");
	...
	private UpgradeServiceWrapperHooksOp _op() {
		return getLocalModelElement().nearest(UpgradeServiceWrapperHooksOp.class);
	}

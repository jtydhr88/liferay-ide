...
public class TomcatBundleConfiguration extends PortalBundleConfiguration
{
    ...
    private List<LiferayServerPort> getPortsValue( Document document, String tagName, String portName, String protocolName  )
    {
        ...
        try
        {
            ...
            for( int i = 0; i < connectorNodes.getLength(); i++ )
            {
                ...
                if ( !existed )
                {
                    LiferayServerPort createServerPort = createLiferayServerPort( protocolValue,protocolValue, portValue );
    ...
    public void save( IProgressMonitor monitor ) throws CoreException
    {
        try
        {
            IPath filePath = bundle.getAppServerDir().append( "conf/" ).append("server.xml");
    ...
    public int getHttpPort()
    {
        ...
        if ( isServerDirty == true )
        {
            ...
            if (file.toFile().exists())
            {
                try
                {
                    Document refreshDocument = XMLUtil.getDocumentBuilder().parse(new InputSource(new FileInputStream(file.toFile())));

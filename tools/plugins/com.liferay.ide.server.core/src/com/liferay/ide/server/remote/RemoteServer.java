...
public class RemoteServer extends ServerDelegate implements IRemoteServerWorkingCopy
{
    ...
    public IStatus validate( IProgressMonitor monitor )
    {
        ...
        try
        {
            ...
            {
                monitor.beginTask(
                    NLS.bind( Msgs.validatingConnection, host, getHttpPort() ), IProgressMonitor.UNKNOWN );
            }

            if( !canMakeHttpConnection() )
            {
                return LiferayServerCore.createWarningStatus(
                    NLS.bind( Msgs.serverNotAvailable, host, getHttpPort() ) );
            }

...
echo "Found expect bundle in $liferayWsInStudioDir gradle.properties file.(Successfully)"
else
echo "Can't found expect bundle in $liferayWsInStudioDir gradle.properties file.(Error)"
fi

#7 check liferay workspace home dir
if [ -d "$HOME/$liferayWsInStudioDir" ]; then

...
public class WatchWorkspaceModulesAction extends SelectionProviderAction {
	...
	private void _undeployModuleFromServer(IProject selectedProject) {
		...
		try {
			...
			GradleProject workspaceGradleModel = GradleUtil.getWorkspaceGradleModel(selectedProject);

			if (workspaceGradleModel != null) {
				GradleProject childModel = GradleUtil.getNestedGradleModel(
					workspaceGradleModel, selectedProject.getName());

				if (childModel != null) {
					FileUtil.delete(new File(childModel.getBuildDirectory(), "installedBundleId"));
				}
			}
		}
		catch (Exception e) {

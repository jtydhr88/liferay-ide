...
if exist "%ProgramFiles(x86)%"\%liferay.workspace.home.dir% (echo The liferay-workspace exists.^(Successfully^)) else echo The liferay-workspace doesn't exist.(Error)

::check Studio home dir
if exist "%ProgramFiles(x86)%"\%liferay.developer.studio.home.dir% (echo The liferay-developer-studio exists.^(Successfully^)) else echo The iferay-developer-studio doesn't exist.(Error)

::check bundle.url in gradle.properties
if exist "%ProgramFiles(x86)%"\%liferay.workspace.home.dir%\gradle.properties findstr "%portal.bundle.zip%" "%ProgramFiles(x86)%"\%liferay.workspace.home.dir%\gradle.properties
